	...
	size_t& image_compressed_size)
{
	...
	if (format == 8) {
		...
		param_image.pixel_format = GPUJPEG_4444_F32_P0123_LINEAR;
	}
	else {
	...
	size_t image_compressed_size)
{
	...
	if (format == 8) { //U8 - RGB
		...
		gpujpeg_decoder_set_output_format(g_decoder, GPUJPEG_RGB, GPUJPEG_4444_F32_P0123_LINEAR); //GPUJPEG_444_U8_P012Z
	}
	else {
